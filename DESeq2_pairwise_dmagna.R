...
# Funtion files are loaded from S: or mint. If working locally, make sure to
# copy this directory in your working directory.
...
if (length(files) == 0) {
  stop("No files found in any of the specified directories.")
}
...
rm(files, file, directories)
# DATA IMPORT & Color settings  ---------------------------------------------------
message("\nImporting CountMatrix and coldata files ...")

## coldata ---------------------------------------------------------------------

# For D. magna, ensure that the column tanks are empty or contain NA. 
# Otherwise, the script will use a multifactor model, which is not applicable 
# to the Daphnia test design.
...
message(paste0("\nRemoving low abundant counts from count matrix using the 'relevance threshold' suggested by Verheijen et al 2022."))
...
# excludes all genes that do not have at least 75% (65% in case of only 3 replicates)of their replicates expressed at 1 CPM in
...
perc <- 0.75 # Percent of samples in ONE condition which need a cpm > minCPM - 0.75 equals 3/3 samples!
...
## Load annotation orgDb package -----------------------------------------------
#' 1: Install the AnnotationDbi package which is needed to query the respective 
#' OrgDb packages. e.g. needed for listing OrgDb content via keytypes() or colnames()

## Load AnnotationDbi or install if needed:
message("\nLoading AnnotationDbi and ",orgDb," annotation package ...")
if (!require("AnnotationDbi", quietly = T)) {
  # if AnnoDbi not installed make sure to install it first!
  if (!require("BiocManager", quietly = T)) { install.packages("BiocManager") } else { detach("package:BiocManager", unload = T) }
  # Install AnnoDbi and load afterwards
  BiocManager::install("AnnotationDbi")
  require("AnnotationDbi")
}

## Load orgDb or install if needed:
if (!require(paste0(orgDb), quietly = T, character.only = T)) {
  # If not installed run the installation.
  orgDb_path = paste0("/mnt/data/EconOMICs/Attract/AnnoDbi/",orgDb) %>% paste0(.,".tar.gz")
  message("\nInstalling ",orgDb," annotation package from: ",orgDb_path," ...")
  install.packages(orgDb_path, type="source", repos=NULL)
  require(orgDb, character.only = TRUE )
} else {
  # Pkg already installed
  message("Annotation package '",orgDb,"' already installed!")
}
# To inspect possible columns to extract and keytpyes use:
#columns(get(orgDb))
#keytypes(get(orgDb))


## Build gene annotation object & merge ----------------------------------------
message("Retrieving annotations from ",orgDb," ...")
# ids for which to retrieve the annotation in the orgDb package
GID = rownames(count.matrix.cl)
# columns from which to select anno info
orgDb_col = c("GID",
  columns(get(orgDb)) %>% grep("_blastTopHit",.,value = T) %>% .[c(4,3,5,7)],
  columns(get(orgDb)) %>% grep("_BestDescr",.,value = T) %>% .[c(5,2,4,3,1)],
  columns(get(orgDb)) %>% grep("_eNOG",.,value = T) %>% .[c(3,6,7,9)] 
  )
GeneAnno = AnnotationDbi::select(get(orgDb), keys=GID, columns=orgDb_col, keytype="GID")
row.names(GeneAnno) <- GeneAnno$GID

# Correct data strucutre: 
# grep numeric column names
num_col = grep("identity|evalue|bit_score",colnames(GeneAnno),value = T)
# convert these to numeric values
for(i in num_col){ GeneAnno[,i] <- as.numeric(GeneAnno[,i]) }

## Export Full Annotation Table with some statistics (bit-score, perc.similiarity, e-value, ...)
sub("[.]eg[.]db$","_AnnoTable+stats.csv",orgDb) %>% sub("^org[.]","",.) %>% 
  paste(home,.,sep = "/") %>% write.csv2(GeneAnno,.,row.names = F)

# Merge GeneAnno with result table function
# x: result table
orgDbAnnoFun = function(x, GeneAnno){
  tmp = merge(as.data.frame(x), GeneAnno, by=0, all.x=T)
  row.names(tmp) = tmp[,1]
  return(tmp[,-1])
}
col = colnames(GeneAnno)[c(5,2,6,8,7,11,14)] # columns to annotate 

res.ls    = lapply(res.ls, FUN = orgDbAnnoFun, GeneAnno=GeneAnno[,col])
resLfs.ls = lapply(resLfs.ls, FUN = orgDbAnnoFun, GeneAnno=GeneAnno[,col])
...
if(length(condition) > 2){
  ...
  for(k in names(RES)[-length(RES)]){
    ...
    Y = RESlfs[[k]]
    s.x = subset(X[X$padj <= p, ], abs(log2FC) >= LFcut.x)[,"Gene"] #%>% droplevels(.)
    s.y = subset(Y[Y$padj <= p, ], abs(log2FC) >= LFcut.y)[,"Gene"] #%>% droplevels(.)
