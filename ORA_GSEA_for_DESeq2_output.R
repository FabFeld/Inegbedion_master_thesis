...
GSEA.entrez <- lapply(inputGSEA, function(x){
  x <- x[which(!is.na(x$ENTREZID) & x$ENTREZID %in% univ.entrez), c("ENTREZID","log2FC")]
  # ensure numeric and drop NA log2FC
  x$log2FC <- as.numeric(as.character(x$log2FC))
  x <- x[which(!is.na(x$log2FC)), ]
  # aggregate duplicates by taking the maximum log2FC per ENTREZID
  agg <- aggregate(log2FC ~ ENTREZID, data = x, FUN = function(z) max(z, na.rm = TRUE))
  y <- setNames(agg$log2FC, as.character(agg$ENTREZID))
...
## ORA Parameters ------------
qcut = 0.05      # qvalueCutoff -> to export ALL results!
pcut = 0.05      # pvalueCutoff -> to export ALL results!
...
MyGsePathway <- function(gse.ls, title, top, pcut, padjm, MIN, MAX){
  ...
  gse <- gseReactome(gse.ls, organism = "zebrafish",
                     minGSSize = MIN, maxGSSize = MAX, pvalueCutoff = pcut, 
                     pAdjustMethod = padjm, verbose = TRUE, seed = TRUE, by = "fgsea")
  
  gseR <- setReadable(gse, OrgDb = org.Dr.eg.db, keyType="ENTREZID")
  ...
  if(nrow(as.data.frame(gse)) > 1){
    pdf(paste0(title,"_",gseFun,".pdf"), width = 13, height = top/2, 
        onefile = TRUE, bg = "transparent", fg = "black")
    
    print(dotplot(gse, showCategory = top, x = "GeneRatio", color = "p.adjust",
                  title = paste0(gseFun," - ",title)))
    
    print(dotplot(gse, showCategory = top, split = ".sign", x = "GeneRatio", 
                  color = "p.adjust", title = paste0(gseFun," - ",title)) + 
            facet_grid(.~.sign))
    
    dev.off()  # ← CRITICAL: CLOSE PDF
  }
}            # ← CRITICAL: CLOSE FUNCTION
    ...
    # Network ----
    if(nrow(as.data.frame(gseR)) > 1){
      if(requireNamespace("enrichplot", quietly = TRUE)){
        ptsim <- tryCatch(enrichplot::pairwise_termsim(gseR), error = function(e) NULL)
        if(!is.null(ptsim)){
          print(emapplot(ptsim, color = "p.adjust", showCategory = top))
        }else{
          message("pairwise_termsim failed; trying emapplot(gseR) as fallback...")
          tmp <- tryCatch(emapplot(gseR, color = "p.adjust", showCategory = top), error = function(e) { message("emapplot(gseR) failed: ", e$message); NULL })
          if(!is.null(tmp)) print(tmp)
        }
      }else{
        message("Package 'enrichplot' not installed. Install it to use pairwise_termsim(). Trying emapplot(gseR) ...")
        tmp <- tryCatch(emapplot(gseR, color = "p.adjust", showCategory = top), error = function(e) { message("emapplot(gseR) failed: ", e$message); NULL })
        if(!is.null(tmp)) print(tmp)
      }
    }else{
      message("Skipping emapplot: need at least 2 enriched terms.")
    }
    ...
    # print(
    #   cnetplot(gseR, foldChange = gse.ls, categorySize=~ -log10(gseR$p.adjust), showCategory = top, colorEdge = F)
    # )
    # print(
...
MyGseKEGG <- function(gse.ls, title, top, pcut, padjm, MIN, MAX){
  ...
  # gseKEGG()
  gse <- gseKEGG(gse.ls, organism = "dre", keyType = "ncbi-geneid", 
                 minGSSize = MIN, maxGSSize = MAX, pvalueCutoff = pcut, use_internal_data = F,
                 pAdjustMethod = padjm, verbose = TRUE, seed = T, by = "fgse")
  ...
  if(nrow(as.data.frame(gse)) > 1){
    ...
    if(nrow(as.data.frame(gseR)) > 1){
      if(requireNamespace("enrichplot", quietly = TRUE)){
        ptsim <- tryCatch(enrichplot::pairwise_termsim(gseR), error = function(e) NULL)
        if(!is.null(ptsim)){
          tmp <- tryCatch(emapplot(ptsim, color = "p.adjust", showCategory = top),
                          error = function(e) { message("emapplot(ptsim) failed: ", e$message); NULL })
          if(!is.null(tmp)) print(tmp) else {
            message("emapplot(ptsim) failed; trying emapplot(gseR) as fallback...")
            tmp2 <- tryCatch(emapplot(gseR, color = "p.adjust", showCategory = top),
                             error = function(e) { message("emapplot(gseR) failed: ", e$message); NULL })
            if(!is.null(tmp2)) print(tmp2)
          }
        }else{
          message("pairwise_termsim failed; trying emapplot(gseR) ...")
          tmp <- tryCatch(emapplot(gseR, color = "p.adjust", showCategory = top),
                          error = function(e) { message("emapplot(gseR) failed: ", e$message); NULL })
          if(!is.null(tmp)) print(tmp)
        }
      }else{
        message("Package 'enrichplot' not installed. Trying emapplot(gseR) ...")
        tmp <- tryCatch(emapplot(gseR, color = "p.adjust", showCategory = top),
                        error = function(e) { message("emapplot(gseR) failed: ", e$message); NULL })
        if(!is.null(tmp)) print(tmp)
      }
...
MyGseGO <- function(gse.ls, ont, key, title, top, pcut, padjm, MIN, MAX){
  ...
  # gseGO()
  gse <- gseGO(gse.ls, OrgDb = org.Dr.eg.db, keyType = key, ont = ont, 
               minGSSize = MIN, maxGSSize = MAX, pvalueCutoff = pcut,
               pAdjustMethod = padjm, verbose = TRUE, seed = T, by = "fgse")
